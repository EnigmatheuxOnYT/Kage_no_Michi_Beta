...
class Cinematics:
    def __init__(self):
        ...
        self.cinematics_bgs = {'forest1': self.load("bgs","Fond_Menu_V2_1280p").convert(),
                               ...
                               'ine2' : self.load("bgs", "Fond_Ine_Dojo_Arene_Nuit_1").convert(),
    ...
    def cinematic_01 (self, screen,saved='none'):
        ...
        self.cinematic_frame(screen, 'mgm1', 0, "Mars 1664, ère Edo.")
    ...
    def cinematic_07 (self,screen,saved='none'):
        ...
        if saved == 'none':
            ...
            self.cinematic_frame(screen, "ine1", 3, "Très bien Sensei. Je vais faire un tour dans le dojo pendant ce temps.", "Keiko, tu m'accompagnes ?", kind_info=[['SM','no_weapon'],['KM','no_weapon'],['SH','no_weapon'],1])
            self.cinematic_frame(screen, "ine1", 3, "Désolée, je dois aller faire ma gymnastique.", kind_info=[['KM','no_weapon'],['SM','no_weapon'],['SH','no_weapon'],1, True])
            self.cinematic_frame(screen, "ine1", 3, "Comme tu veux.", kind_info=[['SM','no_weapon'],['KM','no_weapon'],['SH','no_weapon'],1, True])
            ...
            self.cinematic_frame(screen, "ine1", 3, "Takeshi, essaie de prendre plus souvent l'initiative.", "Ainsi, tu pourras te créer encore plus d'opportunités.", kind_info=[['SM','no_weapon'],['KT','no_weapon'],['SH','no_weapon'],3])
            self.cinematic_frame(screen, "ine1", 3, " Bien, prenons une pause.", "Il faut reprendre des forces avant la prochaine séance.", kind_info=[['SM','no_weapon'],['KT','no_weapon'],['SH','no_weapon'],3])
        ...
        self.lowercase = False
        out1,out2 = self.choice_frame(screen,"doj2",[3,3],["Prévenir Hoshida","Les suivre discrètement","Les attaquer"],[['TW','no_weapon'],['TW_H','cin07'],['SM','no_weapon']])
        return out2
    ...
    def cinematic_08 (self,screen, choose=1):
        ...
        if choose == 1:
            ...
            print("Si vous êtes sur la sauvegarde développeur, merci de paramétrer un choix avec la commande /choice 2 [choix]. Sinon, votre sauvegarde est corrompue",choose)
        return choose
        
    def cinematic_09 (self, screen, saved="none"):
        self.music.play(self.music.menu)
        if saved == 'none':
            self.cinematic_frame(screen, "black", 0, "Ainsi, Musashi continue son entraînement avec Sensei Hoshida.", "Chaque jour, il continue à se dépasser, à s'améliorer et à progresser.",)
            self.ecran_noir(screen)
            ...
            self.cinematic_frame(screen, 'ine1', 2, "Devenu samouraï, Musashi décide de partir à la recherche", "d'un village pour s'équiper afin de se préparer au long combat qui l'attend", "contre le clan Takahiro, celui qui a détruit son village natal.", kind_info=[["SM","no_weapon"],["SH","no_weapon"],0])
        elif saved == 'KM':
            self.cinematic_frame(screen, "black", 0, "Ainsi, Musashi continue son entraînement avec Sensei Hoshida.", "Chaque jour, il continue à se dépasser, à s'améliorer et à progresser.",)
            self.ecran_noir(screen)
            self.cinematic_frame(screen, 'ine1', 0, "Un mois plus tard...")
            ...
            self.cinematic_frame(screen, "black", 0, "Ainsi, Musashi et Takeshi continuent leur entraînement avec Sensei Hoshida.", "Chaque jour, il continuent à se dépasser, à s'améliorer et à progresser.",)
            self.ecran_noir(screen)
            self.cinematic_frame(screen, 'ine1', 0, "Un mois plus tard...")
    ...
    def cinematic_10(self, screen, saved="none"):
        ...
        self.music.play(self.music.zen)
        if saved == 'none':
            self.cinematic_frame(screen,'bamboo4', 1, "Un nouveau départ. Je suis enfin devenu un samouraï. Je vais enfin pouvoir", "venger Magome. Allons à la ville d'Aizuwakamatsu,on n'a plus de temps à ", "perdre. Hm? (Un ennemi ?)",  kind_info=["SM", "SM", "no_weapon", "right"])
            self.cinematic_frame(screen,'bamboo4', 1, "Montrez vous ! Je sais que vous vous trouvez à l'intérieur de cette caravane.",  kind_info=["SM","SM", "no_weapon","right"])
            self.ecran_noir(screen)
            self.cinematic_frame(screen, "bamboo4", 2, "Tiens ? Un samouraï ?", kind_info=[['SM','no_weapon'], ['JM','no_weapon'], 2, None, None, ['JM']])
            self.cinematic_frame(screen, 'bamboo4', 2, "Qui êtes-vous ? Que faîtes vous ici ? Expliquez vous !", kind_info=[['SM','no_weapon'], ['JM','no_weapon'], 1])
            self.cinematic_frame(screen, "bamboo4", 2, "Calmez-vous jeune homme. Je ne souhaite pas vous faire de mal. Avant de me","poser des questions, pourriez-vous au moins vous présenter ?", kind_info=[['SM','no_weapon'], ['JM','no_weapon'], 2, None, None, ['JM']])
            self.cinematic_frame(screen, 'bamboo4', 2, "C'est vrai. Veuillez m'excuser de mon impolitesse. Je suis Shikisha Musashi.", "Je viens du village de Magome, et je suis récemment devenu un samouraï.","Enchanté. Et vous ?", kind_info=[['SM','no_weapon'], ['JM','no_weapon'], 1])
            self.cinematic_frame(screen, 'bamboo4', 2, "Je suis Juzo Ma, un marchand. Je collecte de nombreux produits pour pouvoir", "les revendre ensuite. Voudriez-vous bien jeter un coup d'œil à mes","marchandises ? ", kind_info=[['SM','no_weapon'], ['JM','no_weapon'], 2])
            self.cinematic_frame(screen, 'bamboo4', 2, "Très bien. Je suis intéressé par vos offres. Que vendez-vous ?", kind_info=[['SM','no_weapon'], ['JM','no_weapon'], 1])
            self.cinematic_frame(screen, 'bamboo4', 2, "Je vends de tout. Talismans, potions, nourriture, etc. Auriez-vous une","préférence?", kind_info=[['SM','no_weapon'], ['JM','no_weapon'], 2])
            self.cinematic_frame(screen, 'bamboo4', 2, "Dans ce cas là, vous devez avoir des armes, n'est-ce pas ?", kind_info=[['SM','no_weapon'], ['JM','no_weapon'], 1])
            self.cinematic_frame(screen, 'bamboo4', 2, "Absolument! Actuellement, je possède une arme très destructrice. Le “Tengoku", "No Ikari”.(La colère du Paradis)", kind_info=[['SM','no_weapon'], ['JM','no_weapon'], 2])
            self.cinematic_frame(screen, 'bamboo4', 2, "“Tengoku no Ikari”? Je n'en ai jamais entendu parler. Pourquoi porte-elle","ce nom ?", kind_info=[['SM','no_weapon'], ['JM','no_weapon'], 1])
            self.cinematic_frame(screen, 'bamboo4', 2, "La légende raconte que cette lame incarnerait le châtiment de Dieu. On la", "classait comme l'une des 7 épées légendaires. Tous ceux qui sont touchés", "par cette arme périssent en voyant leurs âmes séparées de leurs corps. ", kind_info=[['SM','no_weapon'], ['JM','no_weapon'], 2])
            self.cinematic_frame(screen, 'bamboo4', 2, "C'est pour cela que je la vends pour 100 pièces d'or.", "Elle est très rare, et possède beaucoup de valeur.", kind_info=[['SM','no_weapon'], ['JM','no_weapon'], 2])
        elif saved == 'KM':
            self.cinematic_frame(screen,'bamboo4', 2, "Le moment est enfin arrivé. Je suis enfin devenu un samouraï.",   kind_info=[['SM','no_weapon'], ['KM','no_weapon'], 1])
            self.cinematic_frame(screen,'bamboo4', 2, "Oui, tu es resté déterminé et tu as réussi à atteindre tes objectifs. Tu es","incroyable grand frère !",   kind_info=[['SM','no_weapon'], ['KM','no_weapon'], 2])
            self.cinematic_frame(screen,'bamboo4', 2, "Merci Keiko, mais c'est aussi grâce à toi que tu me pousses à continuer ce", "voyage. Ensemble, la famille Musashi ne périra jamais. ",   kind_info=[['SM','no_weapon'], ['KM','no_weapon'], 1])
            self.cinematic_frame(screen,'bamboo4', 2, "Très bien dit Shikisha. Ne restons pas les bras croisés et poursuivons ce", "voyage !",   kind_info=[['SM','no_weapon'], ['KM','no_weapon'], 2])
            self.cinematic_frame(screen,'bamboo4', 2, "En effet. Allons-y Keiko. Ne perdons pas de temps !",   kind_info=[['SM','no_weapon'], ['KM','no_weapon'], 1])
            self.ecran_noir(screen)
            self.cinematic_frame(screen,'bamboo4', 2, "Tiens, Shikisha. On dirait qu'il y a une caravane au milieu de la route.",   kind_info=[['SM','no_weapon'], ['KM','no_weapon'], 2])
            self.cinematic_frame(screen,'bamboo4', 2, "C'est vrai. Se pourrait-il qu'il y ait un marchand ?",   kind_info=[['SM','no_weapon'], ['KM','no_weapon'], 1])
            self.cinematic_frame(screen, "bamboo4", 0, "(Les buissons bougent derrière la caravane)")
            self.cinematic_frame(screen,'bamboo4', 2, "Hm ? Un ennemi ?. Montrez vous ! Je sais que vous vous trouvez à l'intérieur", "de cette caravane.",   kind_info=[['SM','no_weapon'], ['KM','no_weapon'], 1])
            self.cinematic_frame(screen, "bamboo4", 0, "(Une figure apparaît)")
            self.cinematic_frame(screen, "bamboo4", 3, "Tiens ? Un samouraï ?", kind_info=[['SM','no_weapon'], ['KM','no_weapon'],['JM','no_weapon'], 3, None, ['JM']])
            self.cinematic_frame(screen,'bamboo4', 3, "Qui êtes-vous ? Que faîtes vous ici ? Expliquez vous !",   kind_info=[['SM','no_weapon'], ['KM','no_weapon'],['JM','no_weapon'], 1])
            self.cinematic_frame(screen, "bamboo4", 3, "Calmez-vous jeune homme. Je ne souhaite pas vous faire de mal. Avant", "de me poser des questions, pourriez-vous au moins vous présenter ?", kind_info=[['SM','no_weapon'], ['KM','no_weapon'],['JM','no_weapon'], 3, None, ['JM']])
            self.cinematic_frame(screen,'bamboo4', 3, "C'est vrai. Veuillez m'excuser de mon impolitesse. Je suis Shikisha Musashi.", "Je viens du village de Magome, et je suis récemment devenu un samouraï.","Enchanté. Et vous ?",   kind_info=[['SM','no_weapon'], ['KM','no_weapon'],['JM','no_weapon'], 1])
            self.cinematic_frame(screen,'bamboo4', 3, "Je suis Keiko Musashi, sa sœur. Ravi de vous rencontrer. Et vous ?",   kind_info=[['KM','no_weapon'],['SM','no_weapon'],['JM','no_weapon'], 1, True])
            self.cinematic_frame(screen, "bamboo4", 3, "Je suis Juzo Ma, un marchand. Je collecte de nombreux produits pour pouvoir", "les revendre ensuite. Voudriez-vous bien jeter un coup d'œil à mes", "marchandises ?", kind_info=[['KM','no_weapon'],['SM','no_weapon'],['JM','no_weapon'], 3])
            self.cinematic_frame(screen,'bamboo4', 3, "Shikisha, je pense qu'il pourrait y avoir des armes ou des armures pour", "t'équiper. Voyons voir de plus près ce qu'il pourrait vendre.",   kind_info=[['KM','no_weapon'],['SM','no_weapon'],['JM','no_weapon'], 1])
            self.cinematic_frame(screen,'bamboo4', 3, "Pas bête chère sœur. Marchand Juzo, que vendez-vous?",   kind_info=[['SM','no_weapon'], ['KM','no_weapon'],['JM','no_weapon'], 1, True])
            self.cinematic_frame(screen, "bamboo4", 3, "Je vends de tout. Talismans, potions, nourriture, etc. Auriez-vous une", "préférence?", kind_info=[['SM','no_weapon'], ['KM','no_weapon'],['JM','no_weapon'], 3])
            self.cinematic_frame(screen,'bamboo4', 3, "Dans ce cas là, vous devez avoir des armes, n'est-ce pas ?",   kind_info=[['SM','no_weapon'], ['KM','no_weapon'],['JM','no_weapon'], 1])
            self.cinematic_frame(screen, "bamboo4", 3, "Absolument ! Actuellement, je possède une arme très destructrice. Le", "“Tengoku No Ikari”. (La colère du Paradis)", kind_info=[['SM','no_weapon'], ['KM','no_weapon'],['JM','no_weapon'], 3])
            self.cinematic_frame(screen,'bamboo4', 3, "“Tengoku no Ikari”? Je n'en ai jamais entendu parler. Pourquoi porte-"," elle ce nom ?",   kind_info=[['SM','no_weapon'], ['KM','no_weapon'],['JM','no_weapon'], 1])
            self.cinematic_frame(screen, "bamboo4", 3, "La légende raconte que cette lame incarnerait le châtiment de Dieu. On la", "classe comme l'une des 7 épées légendaires. Tous ceux qui sont touchés", "par cette arme périssent en voyant leurs âmes séparées de leurs corps.  ", kind_info=[['SM','no_weapon'], ['KM','no_weapon'],['JM','no_weapon'], 3])
            self.cinematic_frame(screen, "bamboo4", 3, "C'est pour cela que je la vends pour 100 pièces d'or.", "Elle est très rare, et possède beaucoup de valeur.", kind_info=[['SM','no_weapon'], ['KM','no_weapon'],['JM','no_weapon'], 3])
        elif saved == 'KT':
            self.cinematic_frame(screen,'bamboo4', 2, "Le moment est enfin venu Takeshi. Nous sommes devenus des samouraïs.",   kind_info=[['SM','no_weapon'], ['KT','no_weapon'], 1])
            self.cinematic_frame(screen,'bamboo4', 2, "Après 1 mois d'entraînement acharné, nous allons enfin pouvoir venger Magome.", "Cela ne sera pas facile Musashi. Il faudra continuer à travailler dur","si on veut battre le clan Takahiro.",  kind_info=[['SM','no_weapon'], ['KT','no_weapon'], 2])
            self.cinematic_frame(screen,'bamboo4', 2, "En effet. Il ne faut surtout pas s'arrêter à notre niveau actuel. On peut", "toujours aller plus loin. Pour l'instant, le but, c'est d'atteindre le", "village d'Aizuwakamatsu pour pouvoir bien s'équiper lors du combat final.",   kind_info=[['SM','no_weapon'], ['KT','no_weapon'], 1])
            self.cinematic_frame(screen,'bamboo4', 2, "Utilisons ces pièces à bon escient. Tu es prêt Musashi ? Traversons cette", "forêt !",   kind_info=[['SM','no_weapon'], ['KT','no_weapon'], 2])
            self.cinematic_frame(screen,'bamboo4', 2, "On y va Takeshi ! Ensemble, nous sommes invincibles !",   kind_info=[['SM','no_weapon'], ['KT','no_weapon'], 1])
            self.ecran_noir(screen)
            self.cinematic_frame(screen,'bamboo4', 2, "Sois vigilant Musashi, un ennemi semble être caché dans les parages.",   kind_info=[['SM','no_weapon'], ['KT','no_weapon'], 2])
            self.cinematic_frame(screen,'bamboo4', 2, "Hm ? Un ennemi ?. Cela semble bien être le cas. Il se cache dans cette", "caravane.",   kind_info=[['SM','no_weapon'], ['KT','no_weapon'], 1])
            self.cinematic_frame(screen,'bamboo4', 2, "Je m'en occupe. Montrez vous ! Je sais que vous vous trouvez à l'intérieur", "de cette caravane.",   kind_info=[['SM','no_weapon'], ['KT','no_weapon'], 2])
            self.cinematic_frame(screen, "bamboo4", 3, "Tiens ? Des voyageurs ? Plus spécifiquement des samouraïs ?", kind_info=[['SM','no_weapon'], ['KT','no_weapon'],['JM','no_weapon'], 3, None, ['JM']])
            self.cinematic_frame(screen,'bamboo4', 3, "Qui êtes-vous ? Que faîtes vous ici ? Expliquez vous !",   kind_info=[['SM','no_weapon'], ['KT','no_weapon'],['JM','no_weapon'], 1])
            self.cinematic_frame(screen, "bamboo4", 3, "Calmez-vous jeunes hommes. Je ne souhaite pas vous faire de mal. Avant de", "me poser des questions, pourriez-vous au moins vous présenter ?", kind_info=[['SM','no_weapon'], ['KT','no_weapon'],['JM','no_weapon'], 3, None, ['JM']])
            self.cinematic_frame(screen,'bamboo4', 3, "C'est vrai. Veuillez nous excuser.",   kind_info=[['SM','no_weapon'], ['KT','no_weapon'],['JM','no_weapon'], 1])
            self.cinematic_frame(screen,'bamboo4', 3, "Je m'excuse aussi. Je suis Kurosawa Takeshi, un samouraï. ",   kind_info=[['SM','no_weapon'], ['KT','no_weapon'],['JM','no_weapon'], 2])
            self.cinematic_frame(screen,'bamboo4', 3, "Je suis Shikisha Musashi. Nous provenons tous les deux du village de Magome", "et nous sommes récemment devenus des samouraïs. ",   kind_info=[['SM','no_weapon'], ['KT','no_weapon'],['JM','no_weapon'], 1])
            self.cinematic_frame(screen,'bamboo4', 3, "Enchanté de faire votre connaissance. Et vous ?",   kind_info=[['KT','no_weapon'],['SM','no_weapon'],['JM','no_weapon'], 1, True])
            self.cinematic_frame(screen, "bamboo4", 3, "Je suis Juzo Ma, un marchand. Je collecte de nombreux produits pour pouvoir", "les revendre ensuite. Voudriez-vous bien jeter un coup d'œil à mes", "marchandises ?", kind_info=[['KT','no_weapon'],['SM','no_weapon'],['JM','no_weapon'], 3])
            self.cinematic_frame(screen,'bamboo4', 3, "Un marchand qui vend divers types d'objets ? On a peut-être la possibilité de","s'équiper au préalable.",   kind_info=[['KT','no_weapon'],['SM','no_weapon'],['JM','no_weapon'], 1])
            self.cinematic_frame(screen,'bamboo4', 3, "Je suis du même avis. Très bien. Que vendez-vous?",   kind_info=[['SM','no_weapon'], ['KT','no_weapon'],['JM','no_weapon'], 1, True])
            self.cinematic_frame(screen, "bamboo4", 3, "Je vends de tout. Talismans, potions, nourriture, etc. Auriez-vous une", "préférence?", kind_info=[['SM','no_weapon'], ['KT','no_weapon'],['JM','no_weapon'], 3])
            self.cinematic_frame(screen,'bamboo4', 3, "Dans ce cas là, vous devez avoir des armes, n'est-ce pas ?",   kind_info=[['SM','no_weapon'], ['KT','no_weapon'],['JM','no_weapon'], 1])
            self.cinematic_frame(screen, "bamboo4", 3, "Absolument ! Actuellement, je possède une arme très destructrice. Le", "“Tengoku No Ikari”. (La colère du Paradis)", kind_info=[['SM','no_weapon'], ['KT','no_weapon'],['JM','no_weapon'], 3])
            self.cinematic_frame(screen,'bamboo4', 3, "“Tengoku no Ikari”? Je n'en ai jamais entendu parler. Pourquoi porte-"," elle ce nom ?",   kind_info=[['SM','no_weapon'], ['KT','no_weapon'],['JM','no_weapon'], 1])
            self.cinematic_frame(screen, "bamboo4", 3, "La légende raconte que cette lame incarnerait le châtiment de Dieu. On la", "classe comme l'une des 7 épées légendaires. Tous ceux qui sont touchés", "par cette arme périssent en voyant leurs âmes séparées de leurs corps.", kind_info=[['SM','no_weapon'], ['KT','no_weapon'],['JM','no_weapon'], 3])
            self.cinematic_frame(screen, "bamboo4", 3, "C'est pour cela que je la vends pour 100 pièces d'or.", "Elle est très rare, et possède beaucoup de valeur.", kind_info=[['SM','no_weapon'], ['KT','no_weapon'],['JM','no_weapon'], 3])
        self.cinematic_frame(screen,"bamboo4",3,"Cependant, vous me semblez être de confiance.", "Aussi vous proposais-je un marché.", "Échangeons l'arme contre un service futur que vous me devrez.",kind_info=[['SM','no_weapon'], [saved,'no_weapon'],['JM','no_weapon'], 3])
        self.cinematic_frame(screen,"bamboo4",3,"Qu'en dites-vous ?",kind_info=[['SM','no_weapon'], [saved,'no_weapon'],['JM','no_weapon'], 3])
        
        out1,out2 = self.choice_frame(screen,"bamboo4",[3,4],["Négocier le prix","Refuser","Accepter contre un service","Le tuer"],[['SM','no_weapon'], [saved,'no_weapon'],['JM','no_weapon']])
        return out2
    ...
    def cinematic_11 (self, screen, saved="none", choose=1):
        ...
        if saved == 'none':
            if choose == 1:
                ...
                self.cinematic_frame(screen, 'bamboo2', 2, "Si on se retrouve, n'hésitez pas à me faire part de ce que vous recherchez.", kind_info=[['SM', 'no_weapon'], ['JM', 'no_weapon'], 2])
                self.cinematic_frame(screen, 'bamboo2', 2, "Compris ! On se retrouvera lorsque le moment viendra.",kind_info=[['SM', 'no_weapon'], ['JM', 'no_weapon'], 1])
                self.cinematic_frame(screen, 'bamboo2', 2, "Sur ce, à la prochaine jeune homme ! ",kind_info=[['SM', 'no_weapon'], ['JM', 'no_weapon'], 2])
                ...
                self.cinematic_frame(screen, 'bamboo2', 1,"Les idiots cupides finissent toujours de la même façon.",kind_info=['JM','JM', 'no_weapon', 'right'])
                self.cinematic_frame(screen, 'black', 0,"Les ténèbres envahissent la vision de Musashi. Son voyage s’arrête ici...")
            ...
            if choose == 1:
                ...
                self.cinematic_frame(screen, 'bamboo2', 3, "Le“Tengoku no Ikari”... J'en prendrai bien soin.",kind_info=[['SM', 'no_weapon'],['KM', 'no_weapon'], ['JM', 'no_weapon'], 1, True])
                self.cinematic_frame(screen, 'bamboo2', 3, "Faîtes en bon usage. Vous ne le regretterez pas.",kind_info=[['SM', 'no_weapon'],['KM', 'no_weapon'], ['JM', 'no_weapon'], 3])
                ...
                self.cinematic_frame(screen, 'bamboo2', 3, "En effet, peut-être une prochaine fois.",kind_info=[['SM', 'no_weapon'], ['KM', 'no_weapon'], ['JM', 'no_weapon'], 1, True])
                self.cinematic_frame(screen, 'bamboo2', 3,"Si on se retrouve, n'hésitez pas à me faire part de ce que vous recherchez.",kind_info=[['SM', 'no_weapon'], ['KM', 'no_weapon'], ['JM', 'no_weapon'], 3])
                self.cinematic_frame(screen, 'bamboo2', 3, "Compris ! On se retrouvera lorsque le moment viendra.",kind_info=[['SM', 'no_weapon'], ['KM', 'no_weapon'], ['JM', 'no_weapon'], 1])
                self.cinematic_frame(screen, 'bamboo2', 3, "Sur ce, à la prochaine jeune homme et jeune fille!",kind_info=[['SM', 'no_weapon'], ['KM', 'no_weapon'], ['JM', 'no_weapon'], 3])
                self.cinematic_frame(screen, 'bamboo2', 3,"A la prochaine marchand Juzo! Faîtes très attention lors de votre voyage !",kind_info=[['SM', 'no_weapon'], ['KM', 'no_weapon'], ['JM', 'no_weapon'], 1])
                ...
                self.cinematic_frame(screen, 'bamboo2', 3,"Es-tu si sûr de ça mon garçon ? Je te déconseille fortement de me sous-", "estimer.",kind_info=[['SM', 'no_weapon'], ['KM', 'no_weapon'], ['JM', 'no_weapon'], 3])
                self.cinematic_frame(screen, 'bamboo2', 3,"Je vous retourne la question. Donnez-moi cette arme, et je vous laisserai", "la vie sauve.",kind_info=[['SM', 'no_weapon'], ['KM', 'no_weapon'], ['JM', 'no_weapon'], 1])
                self.cinematic_frame(screen, 'bamboo2', 3,"Ce n’est pas toi.. Ce n’est pas le frère que je connais ! Tu piétines notre", "honneur.. celui de notre famille..", "Papa et maman.. ils auraient honte de toi..",kind_info=[['KM', 'no_weapon'],['SM', 'no_weapon'], ['JM', 'no_weapon'],1, True])
                self.cinematic_frame(screen, 'bamboo2', 3,"Hahaha.. Quelle arrogance. Tu ne sais donc pas à qui tu t’adresses, hein ?","Essaie encore, jeune homme et tu regrettera d'avoir dit de tels propos.",kind_info=[['KM', 'no_weapon'],['SM', 'no_weapon'], ['JM', 'no_weapon'], 3])
                self.cinematic_frame(screen, 'bamboo2', 0,"Avant même que Musashi ne puisse dégainer son arme, une sensation glaciale", "traverse son corps.")
                self.sound.swoosh3.play()
                self.cinematic_frame(screen, 'bamboo2', 0,"Un filet de sang jaillit de sa gorge. Son souffle se coupe. Ses jambes", "flanchent.")
                self.cinematic_frame(screen, 'bamboo2', 0,"Le marchand Juzo le regarde avec un sourire narquois, essuyant la lame d’un", "kunai recouvert du sang de Musashi.")
                self.cinematic_frame(screen, 'bamboo2', 2,"Quelle naïveté... Pensais-tu vraiment pouvoir me prendre quelque chose", "impunément ?","Tu es peut-être un Samouraï, mais je suis à un autre niveau.",kind_info=[['KM', 'no_weapon'], ['JM', 'no_weapon'], 2])
                self.cinematic_frame(screen, 'bamboo2', 2,"Les idiots cupides finissent toujours de la même façon.",kind_info=[['KM', 'no_weapon'], ['JM', 'no_weapon'], 2])
                self.cinematic_frame(screen, 'black', 0,"Les ténèbres envahissent la vision de Musashi. Son voyage s’arrête ici...")
                self.cinematic_frame(screen, 'black', 0,"FIN.")
        elif saved == 'KT':
            if choose == 1:
                ...
                self.cinematic_frame(screen, 'bamboo2', 3, "Le “Tengoku no Ikari”... J'en prendrai bien soin.",kind_info=[['SM', 'no_weapon'], ['KT', 'no_weapon'], ['JM', 'no_weapon'], 1, True])
                self.cinematic_frame(screen, 'bamboo2', 3, "Faîtes en bon usage. Vous ne le regretterez pas.",kind_info=[['SM', 'no_weapon'], ['KT', 'no_weapon'], ['JM', 'no_weapon'], 3])
                ...
                self.cinematic_frame(screen, 'bamboo2', 3, "En effet, peut-être une prochaine fois.",kind_info=[['SM', 'no_weapon'], ['KT', 'no_weapon'], ['JM', 'no_weapon'], 1, True])
                self.cinematic_frame(screen, 'bamboo2', 3,"Si on se retrouve, n'hésitez pas à me faire part de ce que vous recherchez.",kind_info=[['SM', 'no_weapon'], ['KT', 'no_weapon'], ['JM', 'no_weapon'], 3])
                self.cinematic_frame(screen, 'bamboo2', 3, "Compris ! On se retrouvera lorsque le moment viendra.",kind_info=[['SM', 'no_weapon'], ['KT', 'no_weapon'], ['JM', 'no_weapon'], 1])
                self.cinematic_frame(screen, 'bamboo2', 3, "Sur ce, à la prochaine Monsieur le marchand !",kind_info=[['KT', 'no_weapon'],['SM', 'no_weapon'], ['JM', 'no_weapon'], 1, True])
                ...
                self.cinematic_frame(screen, 'bamboo2', 3,"Aucun problème, on peut dire que c'est un cadeau en premier lieu. N'oublie","pas ta promesse et on se reverra aussitôt !",kind_info=[['SM', 'no_weapon'], ['KT', 'no_weapon'], ['JM', 'no_weapon'], 3])
                ...
                self.cinematic_frame(screen, 'bamboo2', 3,"Es-tu si sûr de ça mon garçon ? Je te déconseille fortement de me sous-", "estimer.",kind_info=[['SM', 'no_weapon'], ['KT', 'no_weapon'], ['JM', 'no_weapon'], 3])
                self.cinematic_frame(screen, 'bamboo2', 3,"Je vous retourne la question. Donnez-moi cette arme, et je vous laisserai", "la vie sauve.",kind_info=[['SM', 'no_weapon'], ['KT', 'no_weapon'], ['JM', 'no_weapon'], 1])
                self.cinematic_frame(screen, 'bamboo2', 3,"Tu t'opposes au code Bushido, cesse tes singeries !",kind_info=[['KT', 'no_weapon'],['SM', 'no_weapon'], ['JM', 'no_weapon'],1, True])
                self.cinematic_frame(screen, 'bamboo2', 3,"Hahaha.. Quelle arrogance. Tu ne sais donc pas à qui tu t’adresses, hein ?","Essaie encore, jeune homme et tu regrettera d'avoir dit de tels propos.",kind_info=[['KT', 'no_weapon'],['SM', 'no_weapon'], ['JM', 'no_weapon'], 3])
                self.cinematic_frame(screen, 'bamboo2', 0,"Avant même que Musashi ne puisse dégainer son arme, une sensation glaciale", "traverse son corps.")
                self.sound.swoosh3.play()
                self.cinematic_frame(screen, 'bamboo2', 0,"Un filet de sang jaillit de sa gorge. Son souffle se coupe. Ses jambes", "flanchent.")
                self.cinematic_frame(screen, 'bamboo2', 0,"Le marchand Juzo le regarde avec un sourire narquois, essuyant la lame d’un", "kunai recouvert du sang de Musashi.")
                self.cinematic_frame(screen, 'bamboo2', 2,"Quelle naïveté... Pensais-tu vraiment pouvoir me prendre quelque chose", "impunément ?","Tu es peut-être un Samouraï, mais je suis à un autre niveau.",kind_info=[['KT', 'no_weapon'], ['JM', 'no_weapon'], 2])
                self.cinematic_frame(screen, 'bamboo2', 2,"Les idiots cupides finissent toujours de la même façon.",kind_info=[['KT', 'no_weapon'], ['JM', 'no_weapon'], 2])
                self.cinematic_frame(screen, 'black', 0,"Les ténèbres envahissent la vision de Musashi. Son voyage s’arrête ici...")
                self.cinematic_frame(screen, 'black', 0,"FIN.")
        self.ecran_noir(screen)
    ...
    def cinematic_13(self,screen,saved):
        ...
        if saved=='none':
            ...
            self.cinematic_frame(screen, "mgm8", 2, "..Magome..Notre village natal.",kind_info=[['SM', 'no_weapon'], ['KM', 'no_weapon'], 1])
            ...
            self.cinematic_frame(screen, "mgm8", 2, "..Magome..Notre village natal.",kind_info=[['SM', 'no_weapon'], ['KT', 'no_weapon'], 1])
    ...
    def cinematic_15(self,screen,saved,juzo,in_genocide_route=False):
        ...
        if saved =='none':
            if in_genocide_route==True:
                ...
                self.cinematic_frame(screen, 'bamboo2', 2, "Et pourtant, tu aurais dit la même chose à ces tueurs non ?", kind_info=[['SM','no_weapon'], ['SH','no_weapon'], 2])
                ...
                if output1=='choice':
                    if output2==1:
                        ...
                        self.music.play(self.music.stressfull, 500)
                        ...
                        self.cinematic_frame(screen, "bamboo2", 0, "(SCHLACK !) (Sensei Hoshida se fait transpercer le coeur par Musashi)")
                        self.sound.oof.play()
                        self.cinematic_frame(screen, "bamboo2", 2, " ! ! ! Guah..",kind_info=[['SM', 'no_weapon'], ['SH', 'no_weapon'], 2])
                        self.sound.cough.play()
                        ...
                        self.cinematic_frame(screen, "black", 0, "Musashi s'approche de plus en plus de son plus grand affrontement.")
                        self.cinematic_frame(screen, "black", 0, "Le chef du clan Takahiro, l'attend dans son domaine avec impatience, pour", "pouvoir en finir.")
                        self.cinematic_frame(screen, "black", 0, "Qui en sortira vainqueur ? A suivre dans le prochain épisode..")
                        self.ecran_noir(screen)
            if juzo==True:
                self.music.play(self.music.exploration)
            ...
            if in_genocide_route==True:
                ...
                self.cinematic_frame(screen, 'bamboo2', 3, "Et pourtant, tu aurais dit la même chose à ces tueurs non ?", kind_info=[['KM','no_weapon'],['SM','no_weapon'], ['SH','no_weapon'], 3])
                self.cinematic_frame(screen, 'bamboo2', 3, "Si la violence est le seul moyen de faire changer le monde à ta guise, alors", "j'en ferai bon usage.", kind_info=[['SM','no_weapon'],['KM','no_weapon'], ['SH','no_weapon'], 1, True])
            ...
            if in_genocide_route==True:
                ...
                self.cinematic_frame(screen, 'bamboo2', 3, "Et pourtant, tu aurais dit la même chose à ces tueurs non ?", kind_info=[['KT','no_weapon'],['SM','no_weapon'], ['SH','no_weapon'], 3])
                self.cinematic_frame(screen, 'bamboo2', 3, "Si la violence est le seul moyen de faire changer le monde à ta guise, alors", "j'en ferai bon usage.", kind_info=[['SM','no_weapon'],['KT','no_weapon'], ['SH','no_weapon'], 1, True])
    ...
    def cinematic_18(self,screen,saved):
        ...
        self.cinematic_frame(screen, "tkh1", 2, "Je te félicite d'avoir vaincu mes très puissants subordonnées..Tu es bien", "l'élève de Hoshida.",kind_info=[['SM', 'no_weapon'], ['TK', 'no_weapon'], 2])
    ...
    def cinematic_20(self,screen,saved,route='neutral'):
        ...
        if route=='pacifist':
            ...
            self.sound.ahah.play()
            self.cinematic_frame(screen, "tkh1", 2, "Ha ! Ha ! Ha !",kind_info=[['SM', 'no_weapon'], ['TK', 'no_weapon'], 1])
            self.cinematic_frame(screen, "tkh1", 2, "Ha ! Ha ! Ha ! Ha ! Ha ! Ha ! Ha ! Ha !",kind_info=[['SM', 'no_weapon'], ['TK', 'no_weapon'], 1])
            self.cinematic_frame(screen, "tkh1", 2, "Ha ! Ha ! Ha ! Ha ! Ha ! Ha ! Ha ! Ha ! Ha ! Ha ! Ha ! Ha ! Ha ! Ha !",kind_info=[['SM', 'no_weapon'], ['TK', 'no_weapon'], 1])
            ...
            self.cinematic_frame(screen, "black", 0, "La légende raconte que Musashi a fui son propre village, et a décidé de", "partir vers un autre pays pour débuter une nouvelle vie.")
            self.cinematic_frame(screen, "black", 0, "Des enquêtes ont été mises en place pour la recherche de ce samouraï, où tout", "le monde parle de sa disparition. ")
            self.cinematic_frame(screen, "black", 0, "La honte, le désespoir, la culpabilité de ce jeune homme restera gravé chez", "lui pour le reste de sa vie, voire de l'éternité.")
            self.cinematic_frame(screen, "black", 0, "FIN")
        self.ecran_noir(screen)
